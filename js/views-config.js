...
function cfgIndicator() {
  ...
  return '<div class="dsc-alert dsc-alert--info dsc-mb-md">填报方式三态：<b>需填报</b>（手工录入/导入）、<b>系统自动算</b>（公式计算、界面置灰）、<b>自动取自周报</b>（系统带出、核对确认、允许覆盖留痕）。</div>' +
    ...
    '<span class="dsc-row-toolbar__count">共 ' + INDICATOR_DICT.length + ' 项核心指标</span></div>' +
...
function openIndicatorEditModal(code) {
  ...
  openModal('修改指标口径 · ' + d.code,
    ...
    '</div></div>' +
    '<div class="dsc-alert dsc-alert--info dsc-mt-sm">变更将留痕：操作人、时间、IP、修改前后值，并按生效期间对新报送任务自动生效。</div>',
...
function cfgEnum() {
  ...
  return '<div class="dsc-alert dsc-alert--info dsc-mb-md">枚举值 100% 字典化：供货类型、品类、评级、学历、岗位、国别、共享中心取自基础字典并可维护扩展；字典外取值无法提交。</div>' +
...
function cfgPeriod() {
  ...
  return '<div class="dsc-alert dsc-alert--info dsc-mb-md">双频率报送：<b>周报</b>每周五填报（共享中心/区域总部/项目级）；<b>月报</b>每月底填报（14 家二级单位，每月底后 5 个工作日内齐套）。</div>' +
    '<div class="dsc-row-toolbar">' +
    '<button class="dsc-btn dsc-btn--primary dsc-btn--sm" onclick="toast(\'新增期间：选择周报（每周五）或月报（每月底）\',\'success\')">+ 新增期间</button>' +
...
function cfgTemplate() {
  ...
  return '<div class="dsc-alert dsc-alert--info dsc-mb-md">模板由指标字典自动生成，保证模板与线上字段、校验规则强一致；指标口径调整后模板自动更新。周报 W1~W6 与月报 D1~D6 模板分列。</div>' +
